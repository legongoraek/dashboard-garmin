      ...
      <CardContent>
        ...
        <Stack spacing={1.5}>
          {activities?.map((activity, index) => {
            ...
            return (
              <Box key={activityId || index}>
                <Box
                  component={activityId ? Link : "div"}
                  to={activityId ? `/activities/${activityId}` : undefined}
                  sx={{
                    display: "flex",
                  ...
                  <Box sx={{ flex: 1, minWidth: 0 }}>
                    <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", gap: 1 }}>
                      <Typography fontWeight={800} noWrap>
                        {activity.activityName || activity.activityType?.typeKey || "Actividad"}
                      </Typography>
                      <Typography color="text.secondary" fontSize={13} sx={{ whiteSpace: "nowrap" }}>
                        {activity.startTimeLocal || activity.startTimeGMT || "Sin fecha"}
                      </Typography>
                    </Box>

                    <Stack direction="row" spacing={1} sx={{ mt: 1, flexWrap: "wrap", rowGap: 1 }}>
                      <Chip size="small" variant="outlined" icon={<RouteIcon sx={{ fontSize: 16 }} />} label={activity.distance ? formatDistanceMeters(activity.distance) : "N/A"} />
                      <Chip size="small" variant="outlined" icon={<TimerIcon sx={{ fontSize: 16 }} />} label={activity.duration ? formatSecondsToHoursMinutes(activity.duration) : "N/A"} />
                      {calories && <Chip size="small" variant="outlined" icon={<LocalFireDepartmentIcon sx={{ fontSize: 16 }} />} label={calories} />}
                      {pace && <Chip size="small" variant="outlined" icon={<SpeedIcon sx={{ fontSize: 16 }} />} label={pace} />}
                    </Stack>
                  </Box>
                </Box>
