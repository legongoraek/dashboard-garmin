...
- **P-007 cerrado (commits `1eb7138`, `f7874e2`):** el usuario senalo que la landing publica invitaba a cualquier visitante a "Iniciar sesion" como si su propia cuenta Garmin fuera a funcionar, cuando la app es single-tenant en toda la arquitectura (sin `user_id` en ninguna tabla de Postgres, una sola cookie `garmin_tokens`) y el login en produccion esta bloqueado para CUALQUIERA, incluido el autor (P-006, bloqueo es por IP de Render, no por credenciales). Primera pasada (`1eb7138`): nota aclaratoria bajo el CTA del hero + mensaje de error de login honesto en `garminService.js`/`index.js`. El usuario senalo que el boton seguia apareciendo en navbar y cierre (solo se habia tocado el hero) y que no debia mostrarse en absoluto. Segunda pasada (`f7874e2`): se quito el CTA `/login` de los 3 lugares; el usuario pidio ademas usar el skill `frontend-design` para mejorar la UI/UX, asi que se rediseno `LandingPage.jsx` con identidad propia (paleta trail-log ink/panel/trail-sand/pulse-coral, tipografia Big Shoulders Display + IBM Plex Sans + IBM Plex Mono, grafico SVG de perfil de elevacion como elemento signature) en vez del look generico MUI-azul anterior. Alcance limitado a `LandingPage.jsx` — el theme MUI compartido, el favicon y el dashboard autenticado no se tocaron. Verificado con `npm run verify` (todo verde) y preview local desktop+mobile.
