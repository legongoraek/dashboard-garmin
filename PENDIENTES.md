...
| P-007 | HECHO | P1 | Landing / Login | La landing pública invitaba a cualquier visitante a "Iniciar sesión" como si el login con su propia cuenta Garmin fuera a funcionar — pero la app es single-tenant en toda la arquitectura (sin `user_id` en el schema de Postgres, una sola cookie `garmin_tokens`, un solo IndexedDB) y el login en producción está bloqueado para cualquiera que no sea el autor (P-006). El usuario señaló que esto era engañoso y pidió corregir la landing | Ninguno, ya implementado | — | 2026-09-14 | Commit `1eb7138`, pusheado. `LandingPage.jsx`: nota bajo el CTA del hero aclarando que el dashboard está vinculado a una sola cuenta y no es registro abierto. `garminService.js`/`index.js`: mensaje de error de login corregido (ya no dice "espera unos minutos", que era falso para el caso de bloqueo por IP). Verificado con `npm run verify` (todo verde) y preview local de la landing (screenshot) |
